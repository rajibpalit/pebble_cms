...
<div class="container" style="background-color: white;min-height: 600px">
    <div class="title" style="border-bottom: 5px solid #E5E5E5;">
        <h3><b>Raw Material Stock Reconcile</b></h3>
    </div>
...
//                            exit;
                            ...
                            ?>
                            <tr>
                                <td><input type="text" name="pname" readonly class="form-control example" value="<?php echo $part_name[0]['material_name'] ?> " id="pname_<?php echo $i ?>" onclick="autocompart(this)" onblur="computpart(this)"/></td>
                                <td><input type="text" name="pcode" readonly class="form-control" value="<?php echo $part_name[0]['material_code'] ?>" id="pcode_<?php echo $i ?>"/></td>
                                ...
                                <td><input type="text" name="p_mu" readonly class="form-control" value="<?php echo $p_mu[0]['keyword_value'] ?>" id="p_mu_<?php echo $i ?>"/></td>
                                ...
                                <td class="hide_this"><input type="text" name="stock_id" class="form-control" value="<?php echo $value['stockrm_id'] ?>" id="stockrm_id_<?php echo $i ?>" readonly /></td> 
...
//                print_r($part_name);exit;
            ...
            <div class="form-group" style="alignment-adjust: middle; margin-top: 50px;">        
                <div class="col-sm-offset-2 col-sm-10" style="margin-left: 300px;margin-bottom: 50px; margin-left: 4%;">
                    <button type="button" name="submit" id="submit" class="btn col-sm-3 save-button-color" style="width: 300px;"><b>Save Raw Material Stock</b></button>                  
                    <a href="<?php echo base_url('#'); ?>" class="btn col-sm-3 close-button-color" style="margin-right: 10px;"><b><?php echo (isset($product)) ? 'Back' : 'Close' ?></b></a>                                                        
...
//        console.log(value);
//        exit;
        var data_save = loc + 'update_and_insert_stockrm';
        $.ajax({
            type: 'post',
            url: data_save,
...
//                exit;
                ...
                window.location.href = root + "/pebblescms/home/raw_mat_stock_list";
            }
        });
    });
...
//        var total_qty = parseFloat(quantity) + parseFloat(existing_qty);
        $('#total_quantity_' + i).val(quantity);
        var today = new Date();
...
// iterate through each td based on class and add the values
    ...
    $('#add_row').on('click', function () {
        ...
        $("#distribution_wozerd").each(function () {
            ...
            tds += '<td><input type="text" name="p_mu" readonly class="form-control" value="" id="p_mu_' + i + '"/></td>';
            tds += '<td><input type="text" name="existing_qty" readonly class="form-control price" value="0" id="existing_qty_' + i + '"/></td>';
            tds += '<td><input type="text" name="quantity" class="form-control" value="" id="quantity_' + i + '" onblur="get_line(this)"/></td>';
            tds += '<td><input type="text" name="total_quantity" readonly class="form-control price" value="0" id="total_quantity_' + i + '"/></td>';
...
//                $('#pcode_' + i).val(data.code);
                $('#pcode_' + i).val(data.material_code);
                $('#pid_' + i).val(data.id);
                $('#pid1_' + i).val(data.id);
